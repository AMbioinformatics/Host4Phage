...
tool_path = Path(__file__).parent.parent.joinpath('bin/minced/minced')

def run(args):
	input_file, output_file = args
	oh = open(output_file, 'w')
	#cmd = f'minced {input_file} {output_file} 1> /dev/null 2> /dev/null'
	cmd = f'{tool_path} {input_file} {output_file} 1> /dev/null 2> /dev/null'
	os.system(cmd)
	oh.close()
	return output_file
